...
{	
	...
	content.nameVillages = function()
	{
		...
		buttonDiv.style.padding = '10px';
			...
			saveButton.className = 'btn-default btn-green';
			...
			/*==== save/restore current names ====*/
			var saveNamesButton = document.createElement('button');
			saveNamesButton.className = 'btn-default btn-green';
			saveNamesButton.innerHTML = 'Uložit aktuální názvy';
			saveNamesButton.style.marginLeft = '10px';
			saveNamesButton.onclick = function() { twcheese.saveCurrentVillageNames(); };
			buttonDiv.appendChild(saveNamesButton);

			var restoreNamesButton = document.createElement('a');
			restoreNamesButton.className = 'btn-default';
			restoreNamesButton.innerHTML = 'Obnovit uložené názvy';
			restoreNamesButton.style.marginLeft = '4px';
			restoreNamesButton.onclick = function() { twcheese.restoreVillageNames(); };
			buttonDiv.appendChild(restoreNamesButton);
...
/*==== Save / Restore village names ====*/
	twcheese.saveCurrentVillageNames = function() {
		var saved = {};
		$('.quickedit-vn').each(function() {
			var id   = $(this).data('id');
			var name = $(this).find('.quickedit-label').data('text');
			if (id && name !== undefined) saved[id] = name;
		});
		var count = Object.keys(saved).length;
		if (!count) {
			UI.InfoMessage('Žádné vesnice k uložení.', 3000, 'error');
			return;
		}
		localStorage.setItem('twcheese.savedVillageNames', JSON.stringify(saved));
		UI.InfoMessage('Uloženo ' + count + ' názvů vesnic.', 3000, 'success');
	};

	twcheese.restoreVillageNames = function() {
		var raw = localStorage.getItem('twcheese.savedVillageNames');
		if (!raw) {
			UI.InfoMessage('Žádné uložené názvy nenalezeny.', 3000, 'error');
			return;
		}
		...
		$('.quickedit-vn').each(function() {
			var id        = $(this).data('id');
			var savedName = saved[id];
			if (savedName === undefined) return;
			$(this).find('.rename-icon').click();
			$(this).find('input[type=text]').val(savedName);
			nameSaveButtons.unshift($(this).find('input[type=button]'));
		});
		if (nameSaveButtons.length) {
			saveVillageNames();
		} else {
			UI.InfoMessage('Žádná ze zobrazených vesnic nemá uložený název.', 3000, 'error');
		}
	};
